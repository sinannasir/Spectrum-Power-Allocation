...
#                else:
    ...
    #     if(len(np.where(np.delete(alpha_strategy_all[-1][:,m]*p_strategy_all[-1],agent)==0)[0])!=self.N-1):
    #         state[2] = np.log10((self.noise_var+np.matmul(np.delete(H_all_2[sim][agent,:,m],agent),
    #                                        np.delete(alpha_strategy_all[-1][:,m]*p_strategy_all[-1],agent)))/(self.scale_gain))                                
    #     else:
    #         state[2] = self.input_placer 
    #     state[3] = 10.0 * sum_rate_list_distributed_policy[-1][agent,agent,m]
    #     #interferers
    #     sorted_interferers = np.argsort(H_all_2[sim-1][agent,:,m])[::-1]
    #     sorted_interferers = np.delete(sorted_interferers,np.where(sorted_interferers==agent))
    #     state[(4 + 0 * self.N_neighbors):(4 + 1 * self.N_neighbors)] = np.log10(H_all_2[sim][agent,sorted_interferers[:self.N_neighbors],m]/self.scale_gain_interf)
    #     state[(4 + 1 * self.N_neighbors):(4 + 2 * self.N_neighbors)] = alpha_strategy_all[-1][sorted_interferers[:self.N_neighbors],m]*p_strategy_all[-1][sorted_interferers[:self.N_neighbors]]
    #     state[(4 + 2 * self.N_neighbors):(4 + 3 * self.N_neighbors)] = 0.4 * sum_rate_list_distributed_policy[-1][sorted_interferers[:self.N_neighbors],sorted_interferers[:self.N_neighbors],m]
    #     #interfereds
    #     sorted_interfereds = np.argsort(H_all_2[sim-1][:,agent,m])[::-1]
    #     sorted_interfereds = np.delete(sorted_interfereds,np.where(sorted_interfereds==agent))
    #     state[(4 + 3 * self.N_neighbors):(4 + 4 * self.N_neighbors)] = np.log10(H_all_2[sim-1][sorted_interfereds[:self.N_neighbors],agent,m]/self.scale_gain_interf)
    #     state[(4 + 4 * self.N_neighbors):(4 + 5 * self.N_neighbors)] = np.log10(self.prev_suminterferences[sorted_interfereds[:self.N_neighbors],m]/self.scale_gain)
    #     state[(4 + 5 * self.N_neighbors):(4 + 6 * self.N_neighbors)] = 0.4 * sum_rate_list_distributed_policy[-1][sorted_interfereds[:self.N_neighbors],sorted_interfereds[:self.N_neighbors],m]      
    #     # if PFS 
    #     if self.pfs:
    #         state[(4 + 6 * self.N_neighbors):(5 + 6 * self.N_neighbors)] = 0.4 / weights[-1][agent]
    #         state[(5 + 6 * self.N_neighbors):(5 + 7 * self.N_neighbors)] = 0.4 / weights[-1][sorted_interferers[:self.N_neighbors]]
    #         state[(5 + 7 * self.N_neighbors):(5 + 8 * self.N_neighbors)] = 0.4 / weights[-1][sorted_interfereds[:self.N_neighbors]]
    #     return state
